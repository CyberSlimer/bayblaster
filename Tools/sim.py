...
def run(tiers, player_skill=0.6, verbose=False):
    ...
    while True:
        ...
        if state == 'fly' and rockets > 0 and abs(vy) < 120 and vx > 0 and random.random() < player_skill:
            rockets -= 1
            vx += ROCKET_STR[R]
            vy += ROCKET_STR[R]*0.18
        # dive: skilled player dives on descent
        diving = state == 'fly' and vy < -150 and random.random() < player_skill
        if state == 'fly':
            g = G*(DIVE_GRAV_MULT if diving else 1.0)
            vy += g*DT
            # zone effects
        ...
        for e in ents:
            ...
            if used and k not in ('birds', 'cloud'): continue
...
def batch(tiers, n=300, skill=0.6, label=''):
    ...
    print(f"{label:28s} tiers={tiers} skill={skill}: median={statistics.median(d):6.0f}m  p10={d[int(n*0.1)]:6.0f}  p90={d[int(n*0.9)]:6.0f}  "
          f"coins~{statistics.median(r['coins'] for r in rs):5.0f}  skips~{statistics.mean(r['skips'] for r in rs):4.1f}  t~{statistics.mean(r['t'] for r in rs):4.1f}s sunk={sum(1 for r in rs if r['hull']<=0)}")
